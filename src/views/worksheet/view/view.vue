...
<template>
  <div>
    <Card>
      <!-- 顶部标题 -->
      <p slot="title">
        <Icon type="android-laptop"></Icon>{{!ViewId?'客户视图':'工单视图'}}
      </p>
      <div>
        <Row :gutter="24" v-if="ViewId" style="padding-bottom:10px">
          <Col span="24">
        ...
        <Row :gutter="24">
          <Col span="24">
          <Alert>您可以选择想要的字段在列表中显示，通过在列表中拖放来选择和重新排序。列宽为0即代表自动分配，建议在列数较多时手动分配列宽以保证内容不被换行。</Alert>
          </Col>
        </Row>
...
export default {
  ...
  data() {
    ...
    return {
      ...
      FlagId: this.$router.history.current.meta.FlagId,
