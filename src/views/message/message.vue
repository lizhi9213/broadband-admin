...
<template>
  <div class="message-main-con">
    ...
    <div class="message-content-con">
      <div style="background-color:#f0f0f0; text-align:right; padding:5px">
        <RadioGroup v-model="type" type="button" @on-change="setCurrentMesType">
          <Radio label="-1">全部</Radio>
          <Radio label="0">处理中</Radio>
          <Radio label="1">已完成</Radio>
        </RadioGroup>
        <Button size="mini" @click="allRead">全部标记为已读</Button>
      </div>
      <transition name="view-message">
        <div v-if="showMesTitleList" class="message-title-list-con">
          <Table ref="messageList" :columns="mesTitleColumns" :show-header="false" :data="currentMesList" :no-data-text="noDataText"></Table>
        </div>
      </transition>
...
export default {
          ...
          render: (h, params) => {
            if (this.currentMessageType === 'unread') {
              return h('div', [
                ...
                params.row.type === 'order' ? toOrder(h, params) : null
              ])
  ...
  methods: {
    ...
    setCurrentMesType(type) {
      ...
      if (this.msgType === 'unread') {
        ...
        if (this.type === '-1') {
          ...
          this.currentMesList = this.unreadMesList.filter(item => {
            return item.WorkStatus === this.type
          })
        }
      } else if (this.msgType === 'hasread') {
        this.noDataText = '暂无已读消息'
        if (this.type === '-1') {
          this.currentMesList = this.hasreadMesList
        } else {
          this.currentMesList = this.hasreadMesList.filter(item => {
            return item.WorkStatus === this.type
          })
        }
      } else {
        ...
        if (this.type === '-1') {
          ...
          this.currentMesList = this.recyclebinList.filter(item => {
            return item.WorkStatus === this.type
          })
        }
      }
    ...
    allRead() {
      this.axios
        .get('/apiweb/message_box/readall')
        .then(res => {
          this.getList()
        })
        .catch()
